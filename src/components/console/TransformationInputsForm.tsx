...
export const TransformationInputsForm = forwardRef<
    HTMLFormElement,
    TransformationInputsFormProps
>(function TransformationInputsForm(props, ref) {
    ...
    const subtemplateNames = useMemo(() => {
        return templates
            .map((template) => {
                if (
                    !('name' in template.header) ||
                    typeof template.header.name !== 'string'
                ) {
                    return null;
                }
    ...
    const fields = useMemo(() => {
        const allInputs = templates.flatMap((template) => {
            if (
                !('inputs' in template.header) ||
                !Array.isArray(template.header.inputs)
            ) {
                return [];
            }
