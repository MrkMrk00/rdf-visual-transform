...
export function TransformationsPanel({ close }: { close: VoidFunction }) {
    ...
    const transformationsByPattern = useMemo(() => partitionTransformations(transformations), [transformations]);
    const exportToJson = useTransformationsStore((store) => store.exportToJsonFile);
    ...
    return (
        <div className="relative w-full h-full bg-white flex flex-col gap-2 p-4">
                ...
                <TooltipProvider>
                    <Tooltip>
                        <TooltipTrigger asChild>
                            <Button variant="ghost" onClick={exportToJson} type="button">
                                <ArrowDownIcon />
                            </Button>
                        </TooltipTrigger>
            ...
            {Object.entries(transformationsByPattern).map(([patternName, transformations], index) => (
                <Fragment key={`transformation-group-${index}`}>
                    <h2 className="font-bold py-2">{patternName}</h2>
                            ...
                            >
                                ...
                                <div className="flex gap-1 items-center h-full">
                                    <Button
                                        variant="ghost"
                                        onClick={() => {
                                            editModalRef.current?.open(transformation.id);
                                        }}
                                    >
                                    ...
                                    <Button
                                        variant="ghost"
                                        onClick={() => {
                                            deleteModalRef.current?.open(transformation.id);
                                        }}
                                    >
...
const DeleteTransformationModal = forwardRef<DeleteTransformationHandle>(function DeteleTransformationModal(_, ref) {
...
const EditTransformationModal = forwardRef<EditTransformationHandle>(function EditTransformationModal(_, ref) {
    const [transformationId, setTransformationId] = useState<string | null>(null);
