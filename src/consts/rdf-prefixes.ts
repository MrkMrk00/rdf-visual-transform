...
export const RDF_PREFIX_MAP: Record<string, string> = {
    ...
    'http://www.openlinksw.com/schemas/graphql/intro#': 'gqi',
    ...
    'http://www.openlinksw.com/schemas/sql#': 'sql',
...
const entries = Object.entries(RDF_PREFIX_MAP);

export function shortenIri(fullIri: string) {
    for (const [iriPrefix, value] of entries) {
        if (fullIri.startsWith(iriPrefix)) {
            return `${value}:${fullIri.replace(iriPrefix, '')}`;
        }
    }
